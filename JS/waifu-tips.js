...
function loadWidget(waifuPath, apiPath) {
	...
	//Append 2D girl to the HTML page
	...
	function registerEventListener() {
		...
		$("#waifu-tool .fa-camera-retro").click(() => {
			showMessage("How was it？", 6000, 9);
			Live2D.captureName = "photo.png";
			Live2D.captureFrame = true;
		});
		$("#waifu-tool .fa-info-circle").click(() => {
			open("https://github.com/yang052513/budget");
		});
		$("#waifu-tool .fa-times").click(() => {
			localStorage.setItem("waifu-display", new Date().getTime());
			showMessage("Hope you have a beautiful day.", 2000, 11);
		...
		console.log(re);
		re.toString = () => {
			showMessage("LOLL Seems like you know coding a little bit", 6000, 9);
			return "";
		};
		$(document).on("copy", () => {
			showMessage("Remember reference after copy~", 6000, 9);
		});
		$(document).on("visibilitychange", () => {
			if (!document.hidden) showMessage("Hello Welcome come back～", 6000, 9);
		});
	}
	...
	function welcomeMessage() {
		...
		if (location.href == SiteIndexUrl) { //如果是主页
			...
			if (now > 5 && now <= 7) text = "Good morning~~ Hope you have a good day!";
			else if (now > 7 && now <= 11) text = "Dont seat too long~ or your hamstring will be super tight lol";
			else if (now > 11 && now <= 14) text = "Time for lunch! Pasta for gainzzz";
			else if (now > 14 && now <= 17) text = "I feel sleepy after the carb...";
			else if (now > 17 && now <= 19) text = "Time for dinner~";
			else if (now > 19 && now <= 21) text = "How's your day?";
			else if (now > 21 && now <= 23) text = ["Good night~", "Time to go to bed"];
			else text = "Why are u still awake?";
		} else if (document.referrer !== "") {
			var referrer = document.createElement("a");
			referrer.href = document.referrer;
			var domain = referrer.hostname.split(".")[1];
			if (location.hostname == referrer.hostname) text = `Welcome to<span style="color:#0099cc;">『${document.title.split(" - ")[0]}』</span> I am the host of budget~ `;
			...
			text = `Welcome to<span style="color:#0099cc;">『${document.title.split(" - ")[0]}』</span>I am the host of budget~ `;
		}
	...
	function loadRandModel() {
		...
		$.ajax({
			...
			success: function(result) {
				if (result.textures["id"] == 1 && (modelTexturesId == 1 || modelTexturesId == 0)) showMessage("I dont have any other clothes", 4000, 10);
				else showMessage("How's my new outfit~?", 4000, 10);
